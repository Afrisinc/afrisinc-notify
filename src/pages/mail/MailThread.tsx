...
import { useMyThread, useReplyToMyThread } from "@/hooks/useMailInbox";
...
const MailThread = () => {
  ...
  const { data: thread, isLoading } = useMyThread(threadId);
  const { mutate: sendReply, isPending: sending } =
    useReplyToMyThread(threadId);
  ...
  if (!thread) {
    return (
      <div className="max-w-2xl mx-auto p-8 text-center text-content-secondary">
        Thread not found.
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 flex flex-col h-[calc(100vh-3.5rem)]">
