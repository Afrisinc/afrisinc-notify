...
import { ArrowLeft, Save, Loader2, Monitor, X, ExternalLink } from "lucide-react";
...
function InAppPreview({
  type, title, body, ctaLabel, autoDismiss, dismissAfter, position
}: {
  type: NotificationType;
  title: string;
  body: string;
    ...
    return (
      <div className="w-full h-[340px] bg-slate-100 dark:bg-slate-900 rounded-xl relative overflow-hidden border border-border flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-[220px] p-5 z-10">
          ...
          <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-tight pr-4">{displayTitle}</h3>
          <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-2 leading-relaxed">{displayBody}</p>
          {ctaLabel && (
            <button className="mt-4 w-full bg-primary text-primary-foreground text-[11px] font-semibold rounded-lg py-1.5">
              {ctaLabel}
            </button>
          )}
          <button className="mt-2 w-full text-[11px] text-slate-400 hover:text-slate-600">Dismiss</button>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full h-[340px] bg-slate-100 dark:bg-slate-900 rounded-xl relative overflow-hidden border border-border">
        <div className="absolute top-0 left-0 right-0 bg-primary text-primary-foreground px-4 py-3 flex items-start gap-3">
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-semibold leading-tight">{displayTitle}</p>
            <p className="text-[11px] mt-0.5 opacity-90 leading-relaxed line-clamp-2">{displayBody}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {ctaLabel && (
              <button className="text-[10px] font-semibold bg-white/20 rounded px-2 py-1 whitespace-nowrap">{ctaLabel}</button>
            )}
      ...
      <div className={cn("absolute z-10 w-[200px]", positionClasses[position])}>
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-border p-3">
          <div className="flex items-start gap-2">
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-semibold text-slate-900 dark:text-slate-100 leading-tight">{displayTitle}</p>
              <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1 leading-relaxed line-clamp-2">{displayBody}</p>
              {ctaLabel && (
...
export function InAppEditor({ initialValue, onSave, onCancel, isSaving }: InAppEditorProps) {
  ...
  const typeOptions: { value: NotificationType; label: string; description: string }[] = [
    { value: "toast", label: "Toast", description: "Small non-blocking notification" },
    { value: "banner", label: "Banner", description: "Full-width top-of-page notification" },
    { value: "modal", label: "Modal", description: "Centered overlay requiring action" },
  ];
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="h-full flex flex-col">
        ...
        <div className="flex items-center gap-3 px-6 py-3.5 border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-10">
          ...
          />
          <Badge variant="secondary" className="bg-violet-100 text-violet-700 dark:bg-violet-950/50 dark:text-violet-400 border border-violet-200 dark:border-violet-800 text-[10px] px-2 shrink-0 capitalize">
            ...
            <div className="lg:col-span-3 space-y-5">
              ...
              <Card className="rounded-xl border-border/60">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-semibold text-foreground">Notification Type</CardTitle>
                </CardHeader>
                <CardContent>
                  <FormField
                    control={form.control}
              ...
              <Card className="rounded-xl border-border/60">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm font-semibold text-foreground">Content</CardTitle>
                    <VariableInserter
                      variables={variables}
                ...
                <CardContent className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Title <span className="text-muted-foreground font-normal">({(titleValue || "").length}/80)</span></FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                            ref={(el) => { (field as any).ref(el); (titleRef as any).current = el; }}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Body <span className="text-muted-foreground font-normal">({(bodyValue || "").length}/400)</span></FormLabel>
                        <FormControl>
                          <Textarea
                            {...field}
              ...
              <Card className="rounded-xl border-border/60">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-semibold text-foreground">Action Button <span className="text-muted-foreground font-normal text-xs">(optional)</span></CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Button label</FormLabel>
                          <FormControl>
                            <Input {...field} placeholder="View details" className="text-sm" maxLength={40} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Button URL</FormLabel>
                          <FormControl>
                            <Input {...field} placeholder="https://..." className="text-sm font-mono" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                ...
                <CardContent className="space-y-4">
                  {typeValue === "toast" && (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium">Position</FormLabel>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <div className="flex items-center justify-between">
                          <div>
                            <FormLabel className="text-xs font-medium">Auto dismiss</FormLabel>
                            <p className="text-[11px] text-muted-foreground">Automatically hide after duration</p>
                          </div>
                          <FormControl>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <div className="flex items-center justify-between mb-2">
                            <FormLabel className="text-xs font-medium">Dismiss after</FormLabel>
                            <span className="text-xs font-semibold text-foreground">{field.value}s</span>
                          </div>
                          <FormControl>
                            <Slider
                              min={1}
