...
import { ArrowLeft, Save, Loader2, Bell, Smartphone, Monitor } from "lucide-react";
...
export function PushEditor({ initialValue, onSave, onCancel, isSaving, appName = "YourApp" }: PushEditorProps) {
  ...
  const handleInsertVariable = (varName: string) => {
    if (activeField === "title") {
      ...
      insertVariableAtCursor(titleRef as any, varName, current, (val) => form.setValue("title", val, { shouldDirty: true }));
    } else {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="h-full flex flex-col">
        ...
        <div className="flex items-center gap-3 px-6 py-3.5 border-b border-border bg-card/80 backdrop-blur-sm sticky top-0 z-10">
          ...
          />
          <Badge variant="secondary" className="bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-400 border border-amber-200 dark:border-amber-800 text-[10px] px-2 shrink-0">
        ...
        <div className="flex-1 overflow-auto">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 p-6 max-w-6xl mx-auto">
            ...
            <div className="lg:col-span-3 space-y-5">
              ...
              <Card className="rounded-xl border-border/60">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm font-semibold text-foreground">Notification Content</CardTitle>
                    <VariableInserter
                      variables={variables}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Active field: <span className="font-medium text-foreground capitalize">{activeField}</span>
                  </p>
                </CardHeader>
                <CardContent className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Title <span className="text-muted-foreground font-normal">({(titleValue || "").length}/65)</span></FormLabel>
                        <FormControl>
                          <Input
                            {...field}
                            ref={(el) => {
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Body <span className="text-muted-foreground font-normal">({(bodyValue || "").length}/240)</span></FormLabel>
                        <FormControl>
                          <Textarea
                            {...field}
                            ref={(el) => {
              ...
              <Card className="rounded-xl border-border/60">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm font-semibold text-foreground">Media & Actions <span className="text-muted-foreground font-normal text-xs">(optional)</span></CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <FormField
                    control={form.control}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Icon URL</FormLabel>
                        <FormControl>
                          <Input {...field} placeholder="https://yourapp.com/icon.png" className="text-sm font-mono" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Image URL <span className="text-muted-foreground font-normal">(banner image)</span></FormLabel>
                        <FormControl>
                          <Input {...field} placeholder="https://yourapp.com/banner.jpg" className="text-sm font-mono" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs font-medium">Action URL <span className="text-muted-foreground font-normal">(on click)</span></FormLabel>
                        <FormControl>
                          <Input {...field} placeholder="https://yourapp.com/page" className="text-sm font-mono" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                        ...
                        <tbody>
                          {variables.map((v, i) => (
                            <tr key={v.name} className={cn("border-b border-border last:border-0", i % 2 === 1 && "bg-muted/20")}>
                              <td className="px-3 py-2"><code className="font-mono text-primary font-semibold">{`{{${v.name}}}`}</code></td>
                ...
                <div className="flex rounded-lg border border-border overflow-hidden">
                  <button
                    ...
                    className={cn("px-2.5 py-1 text-[10px] font-medium flex items-center gap-1 transition-colors",
                      previewDevice === "android" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted")}
                  >
                  ...
                  <button
                    ...
                    className={cn("px-2.5 py-1 text-[10px] font-medium flex items-center gap-1 transition-colors border-l border-border",
                      previewDevice === "ios" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted")}
                  >
                  ...
                  <div className="bg-slate-800 rounded-2xl p-1.5">
                    <div className="bg-white dark:bg-slate-700 rounded-xl p-3 shadow-sm">
                      <div className="flex items-start gap-2">
                        {iconUrl ? (
                          <img src={iconUrl} alt="icon" className="h-8 w-8 rounded-md object-cover shrink-0" onError={(e) => (e.currentTarget.style.display = "none")} />
                        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1">
                            <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">{appName}</span>
                            <span className="text-[10px] text-slate-400">{timeStr}</span>
                          </div>
                          <p className="text-[13px] font-semibold text-slate-900 dark:text-slate-100 mt-0.5 leading-tight">
                            {previewTitle || <span className="text-slate-400 font-normal">Notification title</span>}
                          </p>
                          <p className="text-[12px] text-slate-600 dark:text-slate-300 mt-0.5 leading-relaxed line-clamp-2">
                            {previewBody || <span className="text-slate-400">Notification body message...</span>}
                          </p>
                        </div>
                      </div>
                      {imageUrl && (
                        <div className="mt-2 rounded-lg overflow-hidden">
                          <img src={imageUrl} alt="preview" className="w-full h-24 object-cover" onError={(e) => (e.currentTarget.style.display = "none")} />
                        </div>
                      )}
                    </div>
                  </div>
                  <p className="text-[10px] text-muted-foreground text-center">Android notification shade</p>
                </div>
              ) : (
                /* iOS notification */
                <div className="w-[300px] space-y-2">
                  <div className="bg-gradient-to-b from-blue-400 to-blue-600 rounded-2xl p-4 pb-6">
                    ...
                    <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-3">
                      <div className="flex items-start gap-2">
                        {iconUrl ? (
                          <img src={iconUrl} alt="icon" className="h-10 w-10 rounded-xl object-cover shrink-0" onError={(e) => (e.currentTarget.style.display = "none")} />
                        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1">
                            <span className="text-[10px] font-semibold text-white/80 uppercase tracking-wide">{appName}</span>
                            <span className="text-[10px] text-white/60">{timeStr}</span>
                          </div>
                          <p className="text-[13px] font-semibold text-white mt-0.5 leading-tight">
                            {previewTitle || <span className="font-normal opacity-60">Notification title</span>}
                          </p>
                          <p className="text-[12px] text-white/80 mt-0.5 leading-relaxed line-clamp-2">
                            {previewBody || <span className="opacity-60">Notification body...</span>}
                          </p>
                        </div>
                      </div>
                    </div>
                  </div>
                  <p className="text-[10px] text-muted-foreground text-center">iOS lock screen</p>
                </div>
              )}
            </div>
          </div>
